...
roadRouter.get('/obs', async (req, res) => {
  ...
  let requestedTime = timestamp === 'now' ? Date.now() : Date.parse(timestamp);
  ...
  try {
    ...
    const [metaResponse, dataResponse] = await Promise.all([
      fetch(metaURL, { headers, signal: AbortSignal.timeout(config.apiTimeoutMs) }).then((r) =>
        r.json()
      ),
      fetch(dataURL, { headers, signal: AbortSignal.timeout(config.apiTimeoutMs) }).then((r) =>
        r.json()
      ),
    ]);
...
roadRouter.get('/obs/:stationId', async (req, res) => {
  ...
  try {
    ...
    // parse the responses and return the observations
    logger.info(
      `Road observation metadata and data for station ${req.params.stationId} fetched successfully. Parsing responses...`
    );
...
roadRouter.get('/cameras', async (req, res) => {
  ...
  try {
    // fetch actual data from API
    const [metaResponse, dataResponse] = await Promise.all([
      fetch(metaURL, { headers }).then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      }),
      fetch(dataURL, { headers }).then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      }),
    ]);
...
roadRouter.get('/cameras/:stationId', async (req, res) => {
  ...
  try {
    ...
    // parse the responses and return the camera data
    logger.info(
      `Road camera metadata and data fetched successfully for station: ${req.params.stationId}. Parsing responses...`
    );
