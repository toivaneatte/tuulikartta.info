...
const stubCameraData = () => {
  ...
  cy.fixture('camera-station-detail.json').then((stationDetailBody) => {
    ...
    cy.intercept('GET', /\/api\/road\/cameras\/[^/]+(?:\?.*)?$/, stationDetailBody).as(
      'cameraStation'
    );
  });
  ...
  cy.fixture('camera-station-detail-data.json').then((stationDetailDataBody) => {
    ...
    cy.intercept('GET', /\/api\/road\/cameras\/[^/]+\/history(?:\?.*)?$/, stationDetailDataBody).as(
      'cameraStationData'
    );
  });
};
...
describe('main.js (stubbed)', () => {
  ...
  it('should display camera markers on the map when the camera toggle is enabled', () => {
    cy.get('#map .leaflet-control-select-cam', { timeout: 20000 }).should('be.visible').click();
  ...
  it('should open a new window with the camera feed when clicking on a camera marker', () => {
    ...
    cy.get('@windowOpen').its('firstCall.args.0').should('include', '/html/camera.html?station=');
  });
  ...
  it('should request favourites=1 when favourites mode is enabled', () => {
    ...
    cy.wait('@getData', { timeout: 20000 }).its('request.url').should('include', 'favourites=1');
  });
