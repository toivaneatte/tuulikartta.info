...
(function (camera, undefined) {
    ...
    return {
      properties: {
        ...
        nearestWeatherStationId:
          source.nearestWeatherStationId || fallbackProps.nearestWeatherStationId || null,
      },
  ...
  camera.resolveLatestTime = function (station) {
    ...
    const presets =
      station.properties && station.properties.presets ? station.properties.presets : [];
    let latest = null;
  ...
  camera.loadStationDetails = function (stationId, callback) {
    ...
    $.when(fetchWithTimeout(backendSingleCameraUrl))
  ...
  camera.fetchWeatherData = function (stationId, callback) {
    ...
    $.when(fetchWithTimeout(backendUrl))
  ...
  camera.init = function () {
    ...
    $.when(fetchWithTimeout(backendCameraUrl))
      .done(function (backendResponse) {
  ...
  camera.apiCall = function (w, feature) {
    w.onload = function () {
      ...
      let cleanName = feature.properties.name
        .substring(feature.properties.name.indexOf('_') + 1)
        .replaceAll('_', ' ');
      w.document.getElementById('stationName').textContent = `Asema - ${cleanName}`;
      ...
      camera.loadStationDetails(stationId, function (details, error) {
        ...
        // If no nearest station, show message and hide info view
          ...
          w.document.getElementById('noNearestStation').textContent =
            'Lähintä sääasemaa ei löytynyt';
        ...
        if (data && data.presets) {
          for (let i = 0; i < data.presets.length; i++) {
            ...
            result[preset.id] = preset.history
              .map(function (e) {
                return { time: new Date(e.lastModified).getTime(), url: e.imageUrl };
              })
              .sort(function (a, b) {
