...
(function (Tuulikartta, undefined) {
  ...
  fetch('config/color-thresholds.json')
    ...
    .catch(function (err) {
      console.warn('Could not load color-thresholds.json, using defaults:', err);
    });
  ...
  Tuulikartta.resolveWindSpeed = function (windspeed) {
    ...
    if (windspeed < 1) {
      return { code: 'calm', hex: '#ffffff' };
    } else if (windspeed >= 1 && windspeed < 2) {
      return { code: 'light', hex: '#e6f7ff' };
    } else if (windspeed >= 2 && windspeed < 7) {
      return { code: 'moderate', hex: '#ccffcc' };
    } else if (windspeed >= 7 && windspeed < 14) {
      return { code: 'brisk', hex: '#ffff99' };
    } else if (windspeed >= 14 && windspeed < 21) {
      return { code: 'hard', hex: '#ffcc00' };
    } else if (windspeed >= 21 && windspeed < 25) {
      return { code: 'storm', hex: '#ff3300' };
    } else if (windspeed >= 25 && windspeed < 28) {
      return { code: 'severestorm', hex: '#ff0066' };
    } else if (windspeed >= 28 && windspeed < 32) {
      return { code: 'extremestorm', hex: '#cc0099' };
    } else if (windspeed >= 32) {
      return { code: 'hurricane', hex: '#6600cc' };
    } else {
      return 'calm';
    }
  };
  ...
  Tuulikartta.resolveDoseRate = function (dose) {
    ...
    if (dose < 0.1) return '#d4ffff';
    if (dose >= 0.1 && dose < 0.2) return '#ccffcc';
    if (dose >= 0.2 && dose < 0.3) return '#ffff99';
    if (dose >= 0.3 && dose < 0.4) return '#ffcc00';
    if (dose >= 0.4) return '#ff3300';
    return '#ccffcc';
  };
  ...
  Tuulikartta.resolveWawaCode = function (wawa) {
    ...
    if (selectedLanguage === 'en') {
      if (wawa === 0)
        return { short: 'FairWeather', long: '', class: 'textLabelclassGrey', hex: '#ffffff' };
      if (wawa === 10)
        return {
          short: 'Haze',
          long: '',
          class: 'textLabelclassBlackBackgroundWhite',
          hex: '#ffffff',
        };
      if (wawa === 20)
        return {
      ...
      if (wawa === 21)
        return {
          short: 'Rain',
          long: '',
          class: 'textLabelclassBlackBackgroundGreen',
          hex: '#00b430',
        };
      if (wawa === 22)
        return {
          short: 'Drizzle',
          long: '',
          class: 'textLabelclaenssBlackBackgroundYellow',
          hex: '#ffffb3',
        };
      if (wawa === 23)
        return {
      ...
      if (wawa === 24)
        return {
          short: 'Snow',
          long: '',
          class: 'textLabelclassBlackBackgroundBlue',
          hex: '#9cc3fc',
        };
      if (wawa === 25)
        return {
          short: 'FreezingRain',
          long: '',
          class: 'textLabelclassBlackBackgroundPurple',
          hex: '#ff80df',
        };
      if (wawa === 30)
        return {
      ...
      if (wawa === 50)
        return {
          short: 'Drizzle',
          long: '',
          class: 'textLabelclassBlackBackgroundYellow',
          hex: '#ffffb3',
        };
      if (wawa === 51)
        return {
      ...
      if (wawa === 68)
        return {
          short: 'Sleet',
          long: '',
          class: 'textLabelclassBlackBackgroundOrange',
          hex: '#ffbf80',
        };
      if (wawa === 70)
        return {
      ...
      if (wawa === 0)
        return { short: 'Poutaa', long: '', class: 'textLabelclassGrey', hex: '#ffffff' };
      if (wawa === 10)
        return {
          short: 'Utu',
          long: '',
