...
const OSM_TILE_SERVER_URL =
  window.APP_CONFIG?.OSM_TILE_SERVER_URL || 'http://localhost:8080/tile/{z}/{x}/{y}.png';
...
(function (Tuulikartta, undefined) {
  ...
  var emptymarker = [];
  var showForeignObservations = localStorage.getItem('foreignObservations')
    ? localStorage.getItem('foreignObservations')
    : false;
  ...
  window.observationSource = localStorage.getItem('observationSource')
    ? localStorage.getItem('observationSource')
    : 'Näytä vain synop-asemat';
  window.selectedParameter = localStorage.getItem('selectedparameter')
    ? localStorage.getItem('selectedparameter')
    : 'ws_10min';
  ...
  saa.Tuulikartta.showCloudStrikes = localStorage.getItem('showCloudStrikes')
    ? localStorage.getItem('showCloudStrikes')
    : true;
  ...
  var radarLayerOpacity = localStorage.getItem('radarLayerOpacity')
    ? localStorage.getItem('radarLayerOpacity')
    : 80;
  ...
  Tuulikartta.timeTotime = function (epochtime) {
    ...
    return (
      d.getDate() + '.' + (d.getMonth() + 1) + '.' + d.getFullYear() + ' ' + hours + ':' + minutes
    );
  };
  ...
  Tuulikartta.initMap = function () {
    ...
    L.control
      .zoom({
        zoomInTitle: translations[window.selectedLanguage]['zoomIn'],
        zoomOutTitle: translations[window.selectedLanguage]['zoomOut'],
      })
      .addTo(map);
    ...
    var radarControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-select-radar'
        );
        // eventhandler is set in event-handlers.js
    ...
    var lightningControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-select-flash'
        );
        // eventhandler is set in event-handlers.js
    ...
    var tableDataControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-select-table'
        );
    ...
    var trafficCamControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-select-cam'
        );
    ...
    var favouritesControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-select-favourites'
        );
    ...
    var infoControl = L.Control.extend({
      ...
      onAdd: function (map) {
        var container = L.DomUtil.create(
          'div',
          'leaflet-bar leaflet-control leaflet-control-custom leaflet-control-toggle-info'
        );
        // eventhandler is set in event-handlers.js
  ...
  Tuulikartta.drawData = function (param) {
    ...
    if (!saa.Tuulikartta.showStationObservations && !saa.Tuulikartta.showRoadObservations)
      return false;
    ...
    for (var i = 0; i < sizeofdata; i++) {
      ...
      var location = {
        lat: parseFloat(saa.Tuulikartta.data[i]['lat']),
        lng: parseFloat(saa.Tuulikartta.data[i]['lon']),
      };
      ...
      if (param == 'ws_10min' || param === 'wg_10min') {
        ...
        if (
          saa.Tuulikartta.data[i]['type'] !== 'radiation' &&
          saa.Tuulikartta.data[i]['ws_10min'] !== null &&
          saa.Tuulikartta.data[i]['wd_10min'] !== null &&
          saa.Tuulikartta.data[i]['wg_10min'] !== null
        ) {
          if (saa.Tuulikartta.data[i][param] < 10) {
            var iconAnchor = [30, 28];
          }
          if (saa.Tuulikartta.data[i][param] >= 10) {
            var iconAnchor = [25, 28];
          }

          var icon = L.icon({
            iconUrl:
              '../symbols/wind/' +
              saa.Tuulikartta.resolveWindSpeed(saa.Tuulikartta.data[i][param]).code +
              '.svg',
          ...
          marker.bindPopup(
            saa.Tuulikartta.populateInfoWindow(
              saa.Tuulikartta.data[i],
          ...
          var marker = L.marker(
            new L.LatLng(saa.Tuulikartta.data[i]['lat'], saa.Tuulikartta.data[i]['lon']),
            {
              interactive: false,
              keyboard: false,
              icon: Tuulikartta.createLabelIcon(
                'textLabelclass',
                parseFloat(saa.Tuulikartta.data[i][param]).toFixed(1)
              ),
            }
          );
      ...
      if (param === 'ws_1d' || param === 'wg_1d') {
        ...
        if (
          saa.Tuulikartta.data[i]['type'] !== 'radiation' &&
          saa.Tuulikartta.data[i]['ws_1d'] != null &&
          saa.Tuulikartta.data[i]['ws_max_dir'] != null &&
          saa.Tuulikartta.data[i]['wg_max_dir'] != null &&
          saa.Tuulikartta.data[i]['wg_1d'] != null
        ) {
          if (saa.Tuulikartta.data[i][param] < 10) {
            var iconAnchor = [30, 28];
          }
          ...
          if (param == 'ws_1d') {
            var marker = L.marker(
              ...
              {
                icon: icon,
                rotationAngle: Tuulikartta.resolveWindDirection(
                  saa.Tuulikartta.data[i]['ws_max_dir']
                ),
                rotationOrigin: 'center center',
              }
            );
            ...
            var marker = L.marker(
              [saa.Tuulikartta.data[i]['lat'], saa.Tuulikartta.data[i]['lon']],
              {
                icon: icon,
                rotationAngle: Tuulikartta.resolveWindDirection(
                  saa.Tuulikartta.data[i]['wg_max_dir']
                ),
                rotationOrigin: 'center center',
              }
            );
      ...
      if (param === 'rr_1h' || param === 'ri_10min' || param === 'rr_1d') {
        if (parseFloat(saa.Tuulikartta.data[i][param]) > 0) {
          ...
          hex = 'hex' + hex;
          if (
            saa.Tuulikartta.data[i][param] !== null &&
            parseFloat(saa.Tuulikartta.data[i][param]) > 0
          ) {
            var marker = L.marker(
              new L.LatLng(saa.Tuulikartta.data[i]['lat'], saa.Tuulikartta.data[i]['lon']),
              {
        ...
        if (
          parseFloat(saa.Tuulikartta.data[i][param]) == 0 &&
          saa.Tuulikartta.data[i][param] !== 'NaN'
        ) {
          var marker = L.marker(
            new L.LatLng(saa.Tuulikartta.data[i]['lat'], saa.Tuulikartta.data[i]['lon']),
            {
      ...
      if (param === 'air_activity') {
        if (saa.Tuulikartta.data[i]['type'] === 'air_radio') {
          ...
          if (stationData['Pb-210'] !== null && stationData['Pb-210'] !== undefined) {
            labelHtml +=
              '<div>Pb-210: ' + Tuulikartta.formatNuclide(stationData['Pb-210']) + '</div>';
          }
          ...
          if (stationData['Cs-137'] !== null && stationData['Cs-137'] !== undefined) {
            labelHtml +=
              '<div>Cs-137: ' + Tuulikartta.formatNuclide(stationData['Cs-137']) + '</div>';
          }
          ...
          marker.addTo(saa.Tuulikartta.markerGroupSynop);
          marker.bindPopup(saa.Tuulikartta.populateInfoWindow(stationData, stationData['fmisid']), {
            maxWidth: maxWidth,
          });
      ...
      if (param === 'dewpoint' || param === 't2m' || param === 'tmin' || param === 'tmax') {
        ...
        svgicon =
          svgicon +
          '<g><path d="M20.4,48.8c6.7,1.3,12.5-3.8,12.5-10.3c0-3.4-1.7-6.5-4.3-8.4V7.2c0-3.4-2.7-6.2-6.2-6.2c-3.4,0-6.2,2.8-6.2,6.2v22.9   c-3.1,2.3-4.9,6.2-4,10.6C13,44.7,16.3,48,20.4,48.8z M17.5,32l0.9-0.7v-24c0-2.2,1.8-3.9,3.9-3.9c2.2,0,4,1.8,4,3.9v24l0.9,0.7   c2.1,1.5,3.3,4,3.3,6.6c0,2.2-0.9,4.3-2.4,5.8c-1.6,1.5-3.6,2.4-5.8,2.4c-4.1,0-7.3-2.9-8-6.5C13.8,37,15,33.8,17.5,32z"></path>';
        svgicon =
          svgicon +
          '<path d="M22.4,44.4c1.6,0,3.1-0.6,4.2-1.7c1.1-1.1,1.7-2.6,1.7-4.2c0-4-3.4-5.3-4.3-6.1V15.5h-3.3v16.9c-0.7,0.8-5.1,2.4-4.1,7.3   C17.1,42.3,19.4,44.4,22.4,44.4z" stroke="black" fill="' +
          fillColor +
          '"></path>';
        svgicon =
          svgicon +
        ...
        svgicon =
          svgicon +
          '<path d="M38.1,21c0-0.6-0.5-1.1-1.1-1.1h-5.7v2.3h5.7C37.6,22.2,38.1,21.6,38.1,21z"></path>';
        svgicon = svgicon + '</g>';
        svgicon = svgicon + '</svg>';
        ...
        if (
          saa.Tuulikartta.data[i][param] !== null &&
          Math.abs(saa.Tuulikartta.data[i][param]) < 100
        ) {
      ...
      if (param === 'vis') {
        if (saa.Tuulikartta.data[i]['vis'] !== null) {
          ...
          if (
            parseFloat(saa.Tuulikartta.data[i][param]) < 2000 &&
            parseFloat(saa.Tuulikartta.data[i][param]) >= 1000
          ) {
      ...
      if (param === 'wawa') {
        if (
          saa.Tuulikartta.data[i]['wawa'] !== null &&
          Tuulikartta.resolveWawaCode(saa.Tuulikartta.data[i]['wawa']) !== null
        ) {
          ...
          if (code.short === 'Poutaa' || code.short === 'FairWeather') {
            svgicon =
              svgicon +
              '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" enable-background="new 0 0 50 50" xml:space="preserve">';
            svgicon =
              svgicon +
              `<circle r="5" cx="10" cy="10" stroke="black" stroke-width="2" fill="#ffffff"></circle>`;
            svgicon = svgicon + `</svg>`;
          } else {
            svgicon =
              svgicon +
            ...
            svgicon =
              svgicon +
              `<circle r="5" cx="10" cy="10" stroke="black" stroke-width="2" fill="${code.hex}"></circle>`;
            svgicon = svgicon + `</svg>`;
          }
    ...
    renderedStationIndices.forEach(function (idx) {
      ...
      if (Tuulikartta.isDataStale(data['epochtime'])) {
        var warnMarker = L.marker([parseFloat(data['lat']), parseFloat(data['lon'])], {
          icon: warningIcon,
          interactive: false,
          keyboard: false,
          zIndexOffset: 1000,
  ...
  window.resolveGraphStartposition = function (value) {
    if (value === 'ws_10min' || value === 'wg_10min' || value === 'ws_1d' || value === 'wg_1d')
      ...
      return 2;
    else if (value === 'vis' || value === 'n_man') return 3;
    else if (value === 'dose_rate') return 1;
    else if (value === 'air_activity') return 1;
    else if (value === 'magnetism') return 1;
    else if (value === 'snow_aws') return 4;
    else return 1;
  };
