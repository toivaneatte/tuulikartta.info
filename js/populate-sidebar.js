...
(function (Tuulikartta, undefined) {
  Tuulikartta.populateSidebar = function (radarLayerOpacity) {
    ...
    html +=
      '<input id="show-observations" type="checkbox" checked> ' +
      translations[window.selectedLanguage]['showObservations'];
    html += '<br/>';
    html +=
      '<input id="road-observations" type="checkbox"> ' +
      translations[window.selectedLanguage]['roadObs'];
    html += '<br/>';
    html += '<br/>';
    ...
    html +=
      '<td>' +
      translations[window.selectedLanguage]['radarLayer'] +
      ':</td><td><input type="range" id="radar-opacity" name="opacity" min="0" max="100" value="' +
      radarLayerOpacity +
      '"></td>';
    html += '</tr>';
    html += '</table>';
    ...
    if (saa.Tuulikartta.showCloudStrikes == true || saa.Tuulikartta.showCloudStrikes == 'true') {
      html +=
        ...
        translations[window.selectedLanguage]['allObs'] +
        '</option>';
      html +=
        '<option value="0">' + translations[window.selectedLanguage]['groundOnly'] + '</option>';
    } else {
      html += '<option value="1">' + translations[window.selectedLanguage]['allObs'] + '</option>';
      html +=
        '<option value="0" selected>' +
        translations[window.selectedLanguage]['groundOnly'] +
        '</option>';
    }
    html += '</select>';
    html += '</td>';
    ...
    html +=
      '<option value="15">15 ' + translations[window.selectedLanguage]['minutes'] + '</option>';
    html +=
      '<option value="30">30 ' + translations[window.selectedLanguage]['minutes'] + '</option>';
    html += '</select>';
    html += '</td>';
    html += '</tr>';
    html += '</table>';
  ...
  Tuulikartta.populateInfoWindow = function (data, fmisid) {
    ...
    // all station types don't currently have translations, so we show 'type' directly
    var stationType =
      '<b>' +
      translations[window.selectedLanguage]['stationType'] +
      ':</b> <span id="station-type">' +
      data['type'] +
      '</span> <br>';
    // note that magnetometer stations have artificial ids
    var stationID = '<b>' + 'ID' + ':</b> <span id="station-id">' + data['fmisid'] + '</span> <br>';
    ...
    output +=
      '<b>' +
      translations[window.selectedLanguage]['observationStation'] +
      ': </b>' +
      displayStationName +
      '<br>';
    output += stationType;
    ...
    if (saa.Tuulikartta.timeValue === 'now') {
      output +=
        '<b>' +
        translations[window.selectedLanguage]['latestObservation'] +
        ': </b>' +
        time +
        '<br>';
    } else {
      output +=
        '<b>' + translations[window.selectedLanguage]['observationTime'] + ': </b>' + time + '<br>';
    }
    output += '</div>';
