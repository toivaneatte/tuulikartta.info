...
((window.latitude = localStorage.getItem('latitude') ? localStorage.getItem('latitude') : 65.69),
  (window.longitude = localStorage.getItem('longitude')
    ? localStorage.getItem('longitude')
    : 25.36),
  (window.zoomlevel = localStorage.getItem('zoomlevel') ? localStorage.getItem('zoomlevel') : 5),
  (window.selectedLanguage = localStorage.getItem('language')
    ? localStorage.getItem('language')
    : 'fi'),
  (selectedParam = 'ws_10min'));

if (window.selectedLanguage === 'fi') $('#language-selector-value').html('EN');
if (window.selectedLanguage === 'en') $('#language-selector-value').html('FI');
...
hash.forEach(function (element) {
  ...
  if (param[0] === 'latlon') {
    ...
    if (typeof parseFloat(lat) === 'number' && parseFloat(lat) >= -90 && parseFloat(lat) <= 90)
      latitude = lat;
    if (typeof parseFloat(lon) === 'number' && parseFloat(lon) >= -180 && parseFloat(lat) <= 180)
      longitude = lon;
    if (typeof parseFloat(zoom) === 'number' && parseFloat(zoom) >= 5 && parseFloat(zoom) <= 12)
      zoomlevel = zoom;
  }
  if (param[0] === 'zoom') {
    var zoom = param[1];

    if (typeof parseInt(zoom) === 'number' && parseInt(zoom) >= 5 && parseInt(zoom) <= 12)
      zoomlevel = zoom;
  }
